...
class AmphibianViewModel : ViewModel() {
    ...
    fun getAmphibianList(){
        viewModelScope.launch {
            //set the loading state at the beginning here
            _status.value = AmphibianApiStatus.LOADING
            try {
//              perform a GET request to download the amphibian data by calling getAmphibians()
                _amphibians.value = AmphibianApi.retrofitService.getAmphibians()
                _status.value = AmphibianApiStatus.DONE
            }catch (e: Exception){
//           In the event of an error, _amphibians should be set to an empty list and _status
                //to AmphibianApiStatus.ERROR
